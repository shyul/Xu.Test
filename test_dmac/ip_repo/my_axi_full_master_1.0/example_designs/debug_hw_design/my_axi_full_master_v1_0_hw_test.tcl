# Runtime Tcl commands to interact with - my_axi_full_master_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/my_axi_full_master_v1_0_include.tcl
...
# Test: LITE_AXI
set wdata_lite_axi [get_init_data 0]
create_hw_axi_txn w_lite_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_lite_axi}
create_hw_axi_txn r_lite_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_lite_axi_addr
run_hw_axi w_lite_axi_addr
run_hw_axi r_lite_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_lite_axi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - LITE_AXI"
} else {
	puts "Data comparison test fail for - LITE_AXI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: FULL_AXI
set wdata_full_axi [get_init_data 1]
create_hw_axi_txn w_full_axi_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_full_axi}
create_hw_axi_txn r_full_axi_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_full_axi_addr
run_hw_axi w_full_axi_addr
run_hw_axi r_full_axi_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_full_axi_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - FULL_AXI"
} else {
	puts "Data comparison test fail for - FULL_AXI, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
